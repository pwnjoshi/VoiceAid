 ...
 *     If STT unavailable (Expo Go) → show text input fallback
...
export default function HomeScreen() {
  ...
  const [sttAvailable, setSttAvailable] = useState(true);  // false = show text fallback
  ...
  useEffect(() => {
    ...
    Voice.onSpeechError = (e) => {
      ...
      // Error 9 = permission denied, error 7 = no match, error 5 = no speech
      if (code === '9') {
        setSttAvailable(false);
        ...
        // Unknown error — fall back to text input
        setSttAvailable(false);
  ...
  const handlePress = async () => {
    ...
    if (!sttAvailable) return; // text input handles submission
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <ScrollView contentContainerStyle={styles.scroll} bounces={false} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        ...
        {/* Text input fallback (shown when STT unavailable) */}
        {!sttAvailable && (
